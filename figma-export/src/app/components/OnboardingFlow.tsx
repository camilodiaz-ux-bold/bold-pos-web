    ...
    <div className="flex items-center justify-between px-6 py-5 shrink-0 w-full">
      ...
      <div className="flex gap-5 items-center">
        <div className="bg-white flex gap-1 h-8 items-center justify-center pl-3 pr-2 py-2 rounded-[100px] shadow-[0px_4px_12px_0px_rgba(18,30,108,0.08)] shrink-0">
          <span style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 600, color: '#121E6C' }}>Ayuda</span>
          <BolbotIcon />
        </div>
  ...
  return (
    <div className="flex flex-col gap-5 items-start pb-5 px-6 shrink-0 w-full">
      ...
      <div className="flex h-11 items-center justify-between w-full">
        {onSkip ? (
          <button onClick={onSkip} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 700, color: '#121E6C', textDecoration: 'underline' }}>
            Saltar
          </button>
        ) : onBack ? (
          <button onClick={onBack} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 700, color: '#121E6C', textDecoration: 'underline' }}>
            Atrás
          </button>
        ) : (
          <div />
        ...
        <button
          ...
          style={{
            ...
            backgroundColor: continueEnabled ? '#FF2947' : '#F48990',
    ...
    <div className="flex flex-col" style={{ ...BACKGROUND, width: '100vw', height: '100vh', boxShadow: 'inset 1px 1px 16.1px 0px rgba(18,30,108,0.1)' }}>
      ...
      <div className="flex flex-1 min-h-0 items-center justify-center px-16 gap-16 py-6">
        ...
        <div className="flex flex-col gap-5 flex-1 min-w-0" style={{ maxWidth: 380 }}>
          <span style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 12, color: '#606060', fontWeight: 400 }}>
            {stepLabel}
          </span>
          <h2 style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 28, fontWeight: 700, color: '#121E6C', margin: 0, lineHeight: 1.2 }}>
            {title}
          </h2>
          <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, color: '#1E1E1E', margin: 0, lineHeight: 1.6 }}>
            {description}
          </p>
        </div>
      </div>
...
function StepHeader({ title, subtitle, compact = false }: { title: string; subtitle: string; compact?: boolean }) {
  return (
    <div style={{ marginBottom: compact ? 24 : 36 }}>
      <h2 style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 20, fontWeight: 700, color: '#121E6C', margin: '0 0 8px', lineHeight: 1.3 }}>
        {title}
      </h2>
      <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, color: '#1E1E1E', margin: 0, lineHeight: 1.6 }}>
        {subtitle}
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `2px solid ${selected ? '#FF2947' : 'transparent'}`,
    ...
    >
      {selected && (
        <div style={{
          ...
          width: 20, height: 20, borderRadius: '50%', backgroundColor: '#FF2947',
      ...
      <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 700, color: '#121E6C', margin: '0 0 4px' }}>
        {title}
      </p>
      <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 12, color: '#606060', margin: 0, lineHeight: 1.4 }}>
        {description}
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        border: `2px solid ${selected ? '#FF2947' : '#D2D4E1'}`,
        backgroundColor: selected ? '#FF2947' : 'transparent',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
...
function MerlinInput({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, width: '100%' }}>
      {label && (
        <label style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 600, color: '#121E6C' }}>
      ...
      <input
        ...
        style={{
          ...
          border: `1px solid ${focused ? '#121E6C' : '#E5E7EB'}`,
...
function CityDropdown({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, width: '100%', position: 'relative' }}>
      <label style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 600, color: '#121E6C' }}>
        Ciudad <span style={{ color: '#E63946' }}>*</span>
      </label>
      <div
        onClick={() => setOpen(o => !o)}
        style={{
          height: 40, width: '100%', backgroundColor: '#fff',
          border: `1px solid ${open ? '#121E6C' : '#E5E7EB'}`,
          borderRadius: 12, padding: '0 12px',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          cursor: 'pointer', boxSizing: 'border-box',
        }}
      >
        <span style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, color: value ? '#1E1E1E' : '#606060' }}>
          {value || 'Selecciona una ciudad'}
        </span>
        {open ? <ChevronUp size={18} color="#121E6C" /> : <ChevronDown size={18} color="#121E6C" />}
      </div>
      {open && (
        <div style={{
            ...
            {COLOMBIAN_CITIES.map(city => (
              <div
                ...
                style={{
                  ...
                  backgroundColor: value === city ? '#F7F8FB' : '#fff',
                  fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14,
                  color: '#121E6C', transition: 'background-color 0.1s',
                }}
                onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#F7F8FB')}
                onMouseLeave={e => (e.currentTarget.style.backgroundColor = value === city ? '#F7F8FB' : '#fff')}
              >
                {city}
              </div>
            ))}
      ...
      {options.map(opt => (
        <button
          ...
          style={{
            ...
            backgroundColor: selected === opt ? '#121E6C' : '#F0F1F5',
            color: selected === opt ? '#fff' : '#606060',
            fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14,
...
function ZoneChip({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <div style={{
      ...
      backgroundColor: '#F0F1F5', borderRadius: 100,
      padding: '6px 12px', height: 36,
    }}>
      <span style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, color: '#1E1E1E' }}>{label}</span>
  ...
  return (
    <div
      style={{
        ...
        background: 'linear-gradient(135deg, #0F1729 0%, #1a2f72 60%, #EE424E 100%)',
        cursor: 'pointer',
      }}
    ...
    >
      <div style={{ position: 'absolute', inset: 0, opacity: 0.15 }}>
        ...
        <div style={{ position: 'absolute', bottom: '20%', right: '15%', width: 80, height: 80, borderRadius: '50%', backgroundColor: '#FF2947', filter: 'blur(30px)' }} />
      </div>
  ...
  return (
    ...
    >
      <div style={{
        ...
        <Smartphone size={22} color="#121E6C" />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 600, color: '#121E6C', margin: '0 0 2px' }}>
          Descarga la App Bold POS
        </p>
        <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 12, color: '#606060', margin: 0 }}>
          Escanea el QR y gestiona tu negocio desde el celular
        </p>
      </div>
      <ChevronRight size={18} color="#606060" style={{ flexShrink: 0 }} />
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 20, fontWeight: 700, color: '#121E6C', margin: 0 }}>
          ¿Salir del onboarding?
        </h3>
        <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, color: '#606060', margin: 0 }}>
          Perderás el progreso de la configuración. Puedes retomarlo más tarde.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          <button onClick={onCancel} style={{ padding: '10px 20px', borderRadius: 32, border: '1px solid #E5E7EB', backgroundColor: '#fff', cursor: 'pointer', fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, color: '#121E6C', fontWeight: 600 }}>
            Cancelar
          </button>
          <button onClick={onConfirm} style={{ padding: '10px 20px', borderRadius: 32, border: 'none', backgroundColor: '#FF2947', cursor: 'pointer', fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, color: '#fff', fontWeight: 700 }}>
            Salir
          </button>
        </div>
      </div>
      ...
      {step === 'step-3-1' && (
        <StepWrapper onBack={goBack} onContinue={goNext} continueEnabled percentages={percentages}>
          ...
          <div style={{ marginBottom: 32 }}>
            <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 600, color: '#1E1E1E', margin: '0 0 4px' }}>
              Propina sugerida
            </p>
            <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 12, color: '#606060', margin: '0 0 12px' }}>
              Se mostrará como opción por defecto al cobrar.
            </p>
            <ToggleGroup options={['0%', '10%', '15%']} selected={propina} onSelect={setPropina} />
            <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 12, color: '#9CA3AF', margin: '8px 0 0' }}>
            ...
            {addingZona ? (
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <input
                  ...
                  style={{
                    flex: 1, height: 36, borderRadius: 100, border: '1px solid #E5E7EB',
                    padding: '0 14px', fontSize: 14, outline: 'none',
                    fontFamily: 'Montserrat, Inter, sans-serif',
                  }}
                />
                <button onClick={addZona} style={{ padding: '6px 14px', borderRadius: 100, border: 'none', backgroundColor: '#121E6C', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Montserrat, Inter, sans-serif' }}>
                  Agregar
                </button>
                <button onClick={() => setAddingZona(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#606060' }}>
                  <X size={16} />
                </button>
              </div>
            ) : (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', color: '#121E6C', fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 14, fontWeight: 600, padding: 0 }}
              >
                <Plus size={16} />
                Agregar zona
              </button>
        ...
        <div className="flex flex-col" style={{ ...BACKGROUND, width: '100vw', height: '100vh', boxShadow: 'inset 1px 1px 16.1px 0px rgba(18,30,108,0.1)' }}>
          ...
          <div className="flex flex-1 min-h-0 items-center justify-center flex-col" style={{ padding: '0 40px', gap: 20 }}>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 12, color: '#606060', margin: '0 0 8px' }}>
                Paso 4/4
              </p>
              <h2 style={{ fontFamily: 'Montserrat, Inter, sans-serif', fontSize: 26, fontWeight: 700, color: '#121E6C', margin: '0 0 8px', lineHeight: 1.2 }}>

...
const Badge = ({ status }: { status: OrderStatus }) => {
  const styles: Record<OrderStatus, string> = {
    ...
    'LISTA':          'bg-green-100 text-green-700 border-green-200',
...
function AddProductModal({
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm animate-in slide-in-from-bottom-4 sm:zoom-in duration-200 overflow-hidden">
        <div className="flex items-start justify-between p-5 pb-0">
          <div>
            <h3 className="text-base font-bold text-gray-900">{product.name}</h3>
            <p className="text-xs text-blue-600 font-bold mt-0.5">${product.price.toLocaleString()}</p>
            {product.description && (
              <p className="mt-2 line-clamp-2 leading-snug" style={{ color: '#6B7280', fontSize: '14px' }}>
                {product.description}
              </p>
            )}
          </div>
        ...
        <div className="p-5 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={cn(
                  ...
                  quantity > 1 ? 'border-gray-300 hover:border-blue-400 hover:text-blue-600' : 'border-gray-100 text-gray-300 cursor-not-allowed',
                )}
              >
                <Minus size={14} />
              </button>
              <span className="w-8 text-center text-lg font-black text-gray-900">{quantity}</span>
              <button
          ...
          <div>
            <label className="text-xs font-bold text-gray-500 mb-1.5 block">
              Notas <span className="font-medium text-gray-400">(opcional)</span>
            </label>
            <textarea
        ...
        <div className="flex flex-wrap gap-1.5 mt-3 mb-5">
          {NOTE_CHIPS.map(chip => (
            <button
              ...
              className={cn(
                'px-2.5 py-1 rounded-full text-[11px] font-bold border transition-all',
                note.includes(chip) ? 'bg-blue-700 text-white border-blue-700' : 'bg-gray-50 text-gray-600 border-gray-200 hover:border-blue-300 hover:text-blue-600',
              )}
            >
              {chip}
            </button>
    ...
    <>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm animate-in zoom-in duration-200">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 py-2 rounded-lg border border-gray-200 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors">Cancelar</button>
              <button onClick={() => deleteOrder(showDeleteConfirm)} className="flex-1 py-2 rounded-lg bg-red-600 text-white text-sm font-bold hover:bg-red-700 transition-colors">Eliminar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingOrderId && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm animate-in zoom-in duration-200">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Editar nombre</h3>
            <div className="mb-6">
              <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Nombre de la orden</label>
              <input
                type="text"
                autoFocus
                className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-sm font-semibold"
            ...
            <div className="flex gap-3">
              <button onClick={() => setEditingOrderId(null)} className="flex-1 py-2 rounded-lg border border-gray-200 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors">Cancelar</button>
              <button onClick={saveOrderName} className="flex-1 py-2 rounded-lg bg-blue-700 text-white text-sm font-bold hover:bg-blue-800 transition-colors shadow-lg shadow-blue-700/20">Guardar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {subMode === 'Mostrador' && (
        <div className="bg-white border-b border-gray-100 flex items-center px-6 h-12 shrink-0 relative z-[100] shadow-sm">
          <div className="flex items-center h-full min-w-0">
                ...
                <div
                  ...
                  className={cn(
                    ...
                    activeOrderId === order.id
                      ? 'border-blue-700 bg-blue-50/60 text-blue-900 font-bold'
                      : 'border-transparent text-gray-400 hover:text-gray-600 hover:bg-gray-50/70 font-semibold',
                  )}
                >
                  <div className="flex items-center h-full" onClick={() => selectOrder(order.id)}>
                    ...
                    {order.items.length > 0 && <div className="ml-2 w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0" />}
                    {order.hasPendingChanges && <div className="ml-1 w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0" />}
                  </div>
                  <div className="relative ml-2">
                    <button
                      ...
                      className={cn(
                        'p-1 rounded-md hover:bg-blue-100 transition-all',
                        activeOrderId === order.id || orderMenuId === order.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 text-gray-400',
                      )}
                    >
                    ...
                    {orderMenuId === order.id && orderMenuPos && (
                      <>
                        ...
                        <div
                          className="fixed w-44 bg-white rounded-xl shadow-2xl border border-gray-100 z-[200] overflow-hidden py-1 ring-1 ring-black/5 animate-in slide-in-from-top-2 duration-200"
                        ...
                        >
                          ...
                          >
                            <Settings size={14} className="text-gray-400" /> Editar nombre
                          </button>
                          <button
            ...
            <div className="relative h-full flex items-center shrink-0">
              <button
                ...
                className={cn('flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition-all text-xs font-bold whitespace-nowrap', showOrderList ? 'bg-gray-100 text-blue-900' : 'hover:bg-gray-50 text-gray-500')}
              >
                Ver órdenes
                <span className={cn('flex items-center justify-center min-w-[16px] h-4 px-1 rounded text-[9px] font-black', hiddenOrders.length > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500')}>
                  {hiddenOrders.length}
                </span>
                <ChevronDown size={13} className={cn('transition-transform text-gray-400', showOrderList && 'rotate-180')} />
              </button>
              {showOrderList && (
                <>
                  <div className="fixed inset-0 z-[110] bg-transparent" onClick={() => setShowOrderList(false)} />
                  <div className="fixed mt-1 w-52 bg-white rounded-xl shadow-2xl border border-gray-100 z-[200] overflow-hidden py-1 ring-1 ring-black/5 animate-in slide-in-from-top-2 duration-200"
                  ...
                  >
                    {orders.map(order => (
                      <button
                        ...
                        className={cn(
                          'w-full text-left px-4 py-2.5 text-xs hover:bg-gray-50 flex items-center justify-between gap-2',
                          activeOrderId === order.id ? 'text-blue-700 font-black bg-blue-50/60' : 'font-bold text-gray-700',
                        )}
                      >
                        <span className="flex items-center gap-2">
                          #{order.number}
                          {order.items.length > 0 && <div className="w-1.5 h-1.5 rounded-full bg-blue-400" />}
                          {order.hasPendingChanges && <div className="w-1.5 h-1.5 rounded-full bg-amber-400" />}
                        </span>
            ...
            <button
              onClick={addOrder}
              className="ml-2 shrink-0 text-blue-700 hover:text-blue-800 text-xs font-bold flex items-center gap-1 transition-all underline underline-offset-4 decoration-2 decoration-blue-700/30 hover:decoration-blue-800 whitespace-nowrap"
            >
        ...
        {subMode === 'Mesas' ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 gap-6">
            ...
            <div className="text-center">
              <h2 className="text-xl font-bold text-gray-900">Mostrador listo</h2>
              <p className="text-gray-500 mt-1">No tienes órdenes activas en este momento.</p>
            </div>
            <button onClick={addOrder} className="px-6 py-3 bg-blue-700 text-white rounded-xl font-bold flex items-center gap-2 hover:bg-blue-800 transition-all shadow-lg shadow-blue-700/20">
          ...
          <>
            ...
            <div className="w-[400px] bg-white border-l border-gray-200 flex flex-col shadow-[-10px_0_30px_rgba(0,0,0,0.02)] relative shrink-0">
              ...
              <div className="p-6 border-b border-gray-100 flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Orden #{activeOrder.number}</p>
                  </div>
                ...
                {activeOrder.status === 'EN PREPARACIÓN' && activeOrder.sentToKitchenAt && (
                  <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 px-3 py-2 rounded-xl">
                    ...
                    <span className="text-[11px] text-amber-700 font-bold flex-1">
                    ...
                    {activeOrder.hasPendingChanges ? (
                      ...
                      <button
                        ...
                        className="text-[10px] text-amber-600 hover:text-amber-800 font-bold hover:underline flex items-center gap-1"
                      >
                ...
                <div className="flex gap-2">
                  <button className="flex-1 py-2 px-3 border border-gray-200 rounded-lg text-xs font-bold text-gray-600 flex items-center justify-center gap-2 hover:bg-gray-50">
                  ...
                  {activeOrder.status === 'BORRADOR' && (
                    <button
                      ...
                      className={cn(
                        'flex-1 py-2 px-3 rounded-lg text-xs font-bold flex items-center justify-center gap-2 border transition-all',
                  ...
                  {activeOrder.status === 'EN PREPARACIÓN' && activeOrder.hasPendingChanges && (
                    <button
                      ...
                      className="flex-1 py-2 px-3 rounded-lg text-xs font-bold border border-amber-400 text-amber-700 hover:bg-amber-50 flex items-center justify-center gap-2 transition-all"
                    >
                  ...
                  <button
                    ...
                    className="p-2 border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50 relative"
                  >
                    <MoreVertical size={16} />
                    {showActionsMenu && (
                      <>
                        <div className="fixed inset-0 z-[110]" onClick={() => setShowActionsMenu(false)} />
                        <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-2xl border border-gray-100 z-[120] py-1 overflow-hidden animate-in fade-in zoom-in duration-100 origin-top-right">
                          <button
                            onClick={() => { toast.info('Comanda enviada a impresión'); setShowActionsMenu(false); }}
                            className="w-full text-left px-4 py-2.5 text-xs font-bold text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                          >
                            <Printer size={14} className="text-gray-400" /> Reimprimir comanda
                          </button>
                          <button
                            onClick={() => { toast.info('Cuenta enviada a impresión'); setShowActionsMenu(false); }}
                            className="w-full text-left px-4 py-2.5 text-xs font-bold text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                          >
                            <Printer size={14} className="text-gray-400" /> Reimprimir cuenta
                          </button>
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
                {activeOrder.items.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-gray-400 gap-4 mt-20">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-bold text-gray-600">No hay productos aún</p>
                    ...
                    <div key={item.id} className="group flex flex-col gap-2">
                      <div className="flex items-start justify-between">
                        <div className="flex-1 pr-4">
                          ...
                          <p className="text-xs text-blue-600 font-bold mt-0.5">${(item.price * item.quantity).toLocaleString()}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="flex items-center bg-gray-50 rounded-lg border border-gray-100">
                            <button onClick={() => updateItemQuantity(item.id, -1)} className="p-1.5 hover:text-blue-600 transition-colors">
                              <ChevronLeft size={16} />
                            </button>
                      ...
                      {/* Nota del ítem */}
                      <div className="flex flex-col gap-1.5 pl-3 border-l-2 border-gray-100 mt-1">
                        {item.note ? (
                          <div className="flex flex-col gap-1">
                            <div className="flex items-start gap-2">
                              <MessageSquare size={12} className="text-blue-500 mt-0.5 shrink-0" />
                              <p className="text-[11px] text-gray-600 font-medium line-clamp-2">{item.note}</p>
                            </div>
                            <button
                              onClick={() => setEditNoteModal({ itemId: item.id, itemName: item.name, note: item.note ?? '' })}
                              className="text-[10px] text-blue-500 font-bold hover:underline w-fit flex items-center gap-1"
                            >
                          ...
                          <button
                            ...
                            className="text-[10px] text-gray-400 hover:text-blue-500 flex items-center gap-1 w-fit transition-colors"
                          >
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 flex flex-col gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  {activeOrder.isPaid && (
                    <div className="flex items-center gap-2 text-emerald-700 text-xs font-bold bg-emerald-50 border border-emerald-200 p-2 rounded-lg mt-1">
                      <CheckCircle size={14} /> ORDEN PAGADA
                    </div>
                  )}
                  {activeOrder.status === 'ENTREGADA' && !activeOrder.isPaid && (
                    <div className="flex items-center gap-2 text-blue-700 text-xs font-bold bg-blue-50 border border-blue-200 p-2 rounded-lg mt-1">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {activeOrder.status === 'EN PREPARACIÓN' && (
                    <button
                      onClick={markAsReady}
                      className="w-full py-2.5 rounded-xl font-black flex items-center justify-center gap-2 transition-all border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-300"
                    >
                      <CheckCircle2 size={18} className="text-emerald-500" /> Marcar como lista
                    </button>
                  )}
                      ...
                      {activeOrder.isPaid && (
                        <>
                          <button
                            ...
                            className="w-full bg-emerald-600 py-3 rounded-xl text-sm font-bold text-white hover:bg-emerald-700 flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-600/20"
                          >
                            <History size={18} /> Entregar y cerrar
                          </button>
                          <button
                            onClick={() => toast.info('Comprobante enviado a impresión')}
                            className="text-xs font-bold text-blue-600 hover:underline mx-auto flex items-center gap-1"
                          >
                      ...
                      {!activeOrder.isPaid && (
                        <>
                          <button
                            ...
                            className="w-full border border-gray-200 bg-white py-3 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 flex items-center justify-center gap-2 transition-all"
                          >
                            <CheckCircle2 size={18} className="text-emerald-500" /> Marcar como entregada
                          </button>
                          <div className="flex items-center gap-1 text-[10px] text-amber-600 px-1 font-bold">
                  ...
                  {!activeOrder.isPaid && activeOrder.status !== 'CERRADA' && (
                    <button
                      ...
                      className={cn(
                        'w-full py-4 rounded-xl font-black flex flex-col items-center justify-center leading-none transition-all shadow-xl uppercase tracking-widest',
                        activeOrder.items.length === 0
